...
(function (b) {
  var a = function (d, f) {
    ...
    var s = t / 2 / Math.PI;
    o.each(function (A) {
      var C = b(this).height() / 2;
      var x = b(this).width() / 2;
      var B = [];
      for (var y = 0; y < t; y += 1) {
        B.push({
          top: (1 + Math.sin(Math.PI + y / s)) * q - C,
          left: r * (1 + Math.cos(y / s)) - x,
        });
      }
    ...
    var i = function (x) {
      m = x;
      if (m === true) {
        w.addClass("vortex-animating");
        if (u.beforeAnimation) {
          if (u.beforeAnimation.call() === false) {
            return false;
          }
        }
      } else {
        w.removeClass("vortex-animating");
        if (u.afterAnimation) {
          u.afterAnimation.call(null, b(this));
        }
      }
    };
